...
module adder_nbit ( a, b, carry_in, sum, overflow );
  ...
  wire   [3:1] carry_flags;

  adder_1bit_3 \genblk1[0].IX  ( .a(a[0]), .b(b[0]), .carry_in(carry_in), 
        .sum(sum[0]), .carry_out(carry_flags[1]) );
  adder_1bit_2 \genblk1[1].IX  ( .a(a[1]), .b(b[1]), .carry_in(carry_flags[1]), 
        .sum(sum[1]), .carry_out(carry_flags[2]) );
  adder_1bit_1 \genblk1[2].IX  ( .a(a[2]), .b(b[2]), .carry_in(carry_flags[2]), 
        .sum(sum[2]), .carry_out(carry_flags[3]) );
  adder_1bit_0 \genblk1[3].IX  ( .a(a[3]), .b(b[3]), .carry_in(carry_flags[3]), 
